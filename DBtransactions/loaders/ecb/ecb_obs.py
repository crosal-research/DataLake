...
def fetch(tickers: List[str], limit:Optional[str]=None) -> List[List[Observation]]:
    ...
    """
    ntickers = list(set(tickers + ["ECB.EUR/USD"]))
    if limit in [None, '']:
    ...
    dfs_new = ft.reduce(lambda left, right: pd.merge(left, right, left_index=True, right_index=True, how='outer'), dfs)
    
    def _filter_ticker(ticker):
        return (ticker.split(".")[1]).split("/")

    def _new_df(df: pd.DataFrame, ticker):
        ftck = _filter_ticker(ticker)
        if ticker in ["ECB.EUR/USD", "ECB.USD/EUR"]:
            ds = pd.DataFrame(df.loc[: , 'USD'])
            ds.columns = [ticker]
            return ds
        else:
            return pd.DataFrame(df.loc[: , ftck[1]]/df.loc[:, ftck[0]], columns=[ticker])
